...
# Step 2: Prep + submit (on the cluster)
...
**Important caveat:** Starting from a 2D SDF is a much bigger assumption than using a crystal structure PDB. The 3D geometry generated by OpenBabel's force field is a rough approximation — it may not capture the correct conformer, especially for complex macrocycles like vancomycin where the 3D arrangement of the backbone is critical. Geometry optimization (`BP-TZVPD-OPT`) is strongly recommended when starting from SDF to let DFT find a reasonable minimum. Even then, there is no guarantee the optimizer will find the *global* minimum rather than a local one. For high-accuracy work, always prefer experimental structures (X-ray, cryo-EM) when available.
